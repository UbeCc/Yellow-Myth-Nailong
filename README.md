...
- 完整战斗循环：标题 → 开战横幅 → 三段连击攒气势（40/40/70）→ F 重击释放（110/150/190）→ BOSS 半血狂暴（提速 + 蓄力冲锋红圈预警 + 暗影火球，均可翻滚闪避）→ 胜利/死亡结算 → `R` 重开。新增 Q 技能「吐奶弹」：白色追踪奶球，30 伤害，3 秒冷却，右下角有冷却指示。
...
- HUD 小地图（左下）：奶龙朝向刻度、Boss 位置（狂暴变红）、岩石标记。
- 角色方向状态：移动方向倾斜、待机 1.5 秒转身面向镜头（大眼黑瞳白肚皮正脸可见）。
...
- `W/A/S/D`：移动
- `Space`：闪避翻滚（带无敌帧，可躲蓄力冲锋与火球）
- `Q`：吐奶弹（远程，追踪 Boss，3 秒冷却）
- `F`：气势重击（轻击命中攒气势，满 1 格可用，70+40/点，大范围 + 大硬直）
- `Enter` / 鼠标左键：攻击（连按三段连击，第三段为重击）
- `R`：死亡或胜利后重开一局
- 视角：第三人称平滑跟随相机（打击震屏 + 岩石防遮挡）
- 碰撞：角色与岩石有实体碰撞，不再穿模
