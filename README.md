...
- 玩家为程序化生成的黄色奶龙模型（梨形身、短四肢、笑脸），BOSS 为 Quaternius 深色巨龙（CC0）。
- 完整战斗循环：标题 → 开战横幅 → 三段连击（40/40/70）→ BOSS 半血狂暴（提速 + 蓄力冲锋，地面红圈预警）→ 胜利/死亡结算 → `R` 重开。
- 打击感套件：伤害数字（世界坐标投影）、命中顿帧、屏幕震动、受击红晕、NAILONG SMASH 终结技弹字、程序化动画（待机呼吸、攻击前冲、受击脉冲、Boss 悬浮/踉跄、死亡下沉）。
